...
func Test_labelWatchImpl_ValidationV2(t *testing.T) {
	now := time.Now()
	creationTime := &metav1.Time{Time: now.Add(-2 * time.Minute)}
	logf.SetLogger(logf.ZapLogger(true))
	log := logf.Log.WithName("Test_labelWatchImpl_Validation")
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := l.ValidationV2(tt.args.kclient, reqLogger, tt.args.kd, tt.args.dep, tt.args.canaryDep)
			if (err != nil) != tt.wantErr {
				t.Errorf("labelWatchImpl.ValidationV2() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("labelWatchImpl.ValidationV2() = %#v, want %#v", got, tt.want)
			}
		})
	}
}
