...
kubectl -n istio-system wait --timeout=60s --for=condition=complete job/istio-init-crd-10
kubectl -n istio-system wait --timeout=60s --for=condition=complete job/istio-init-crd-11
helm upgrade -i istio istio.io/istio --wait --namespace istio-system -f ${REPO_ROOT}/hack/istio/istio-values.yaml
